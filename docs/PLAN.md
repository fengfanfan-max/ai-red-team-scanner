...
**M4 — 扫描向导 + 扫描列表**
- [x] 创建扫描向导前端（5 步 + store + 成本预估展示）
- [x] 扫描列表页（状态/进度/轮询）+ 空态/错误态
- DoD：E2E：注册→建应用→模拟扫描→列表看到进度到 100%
  - ✅ Playwright E2E（系统 Chrome）：注册→建应用→向导 5 步→启动→列表轮询至 100% + 安全评分，实测通过
  - ✅ 21 前端测试全绿；轮询仅在有活动扫描时开启（2s 节流）

**M5 — 结果页 + Dashboard**
- [x] 结果页（评分总览、类别分布图、失败用例表格 + 详情抽屉）
- [x] Dashboard（统计卡、最近扫描、风险分布图）
- DoD：E2E 主路径完整跑通；空数据态、错误态齐备
  - ✅ E2E 扩展：结果页（评分/类别/失败表格）+ Dashboard（统计卡/最近扫描/风险分布）实测通过
