...
- **AI 应用（AI Application）**：目标模型的接入配置——base_url、api_key、model_name、输入/输出模态。用户自带 API key，属于本团队。
...
- **测试对话（Test Chat）**：扫描启动前与目标模型的一轮试聊，用于验证接入配置可用。
...
- **内置小数据集**：v1 内置 4-6 个风险类别（内容安全/隐私/合规/幻觉等）× 每类 10-20 条提示词，MIT 许可随仓库分发；以 JSON 文件形式随代码发布（非 DB 行），运行时加载。
- **自定义导入**：用户可上传自定义数据集（JSON 格式，含类别/子类目/用例），存 DB，与内置数据集同样参与扫描。
